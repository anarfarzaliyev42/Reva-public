...
let i = 0;
let countOfAnimation = 0;
let textArray = ["Frontend developer.", "Web designer.", "Graphic designer."];
let j = 0;
let tempText = "";
...
function typeWriter() {
  if (j < textArray.length) {
    if (i < textArray[j].length) {
      tempText += textArray[j].charAt(i);

      $($(".anime-text")[0]).text(tempText);
      i++;
...
function reverseType() {
  if (i > 0) {
    tempText = textArray[j].slice(0, i - 1);
    $($(".anime-text")[0]).text(tempText);
    i--;
